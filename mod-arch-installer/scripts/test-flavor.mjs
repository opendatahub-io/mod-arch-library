 ...
 */
function runJest(dir, extraArgs = []) {
  return new Promise((resolve, reject) => {
    console.log(`[test-flavor] Running tests in ${dir}...`);
    const args = ['test', '--passWithNoTests', ...extraArgs];
...
async function testFlavor() {
  ...
  try {
    ...
    // Run tests
    const jestExtraArgs = process.argv.slice(3);
    const testExitCode = await runJest(frontendDir, jestExtraArgs);

    if (testExitCode === 0) {
      console.log('[test-flavor] ✅ All tests passed!');
    } else {
      console.log(`[test-flavor] ❌ Tests failed with exit code ${testExitCode}`);
    }
